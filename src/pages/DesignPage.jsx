...
import { designProjects, getDesignImages } from '../data/designs';
...
const DesignPage = () => {
  ...
  const design = designProjects.find((d) => d.slug === slug);

  useEffect(() => {
    const loadImages = async () => {
      if (design) {
        setLoading(true);
        const designImages = await getDesignImages(design.folder);
        ...
        <div className="container mx-auto px-6 lg:px-12">
          <div className="design-header">
            ...
            <p className="text-xl text-white/60 max-w-2xl">
              Explore the complete design collection for {design.title}. Click on any image to view it in full screen.
            </p>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {images.map((image, index) => (
                <div
                  key={index}
                  className="design-image group cursor-pointer"
